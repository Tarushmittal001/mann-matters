...
/* One colour per tab, so switching tabs visibly changes the temperature of
   the panel rather than only its text. */
const TAB_HUES = ["#0E9FA6", "#F0B429", "#E14D7C"];
...
export default function ManuOnWhatsApp() {
  ...
  return (
    <section className="section relative overflow-hidden bg-forest-950 text-ivory">
      <div className="mesh-cool pointer-events-none absolute inset-0 opacity-80" aria-hidden="true" />
      <div className="wrap-wide relative">
        <div className="grid items-start gap-12 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1fr)] lg:gap-20">
          ...
          <div className="min-w-0 lg:pt-4">
            <Reveal from="right">
              <p className="eyebrow mb-4 flex items-center gap-3 !text-mor-light">
                <span className="font-deva text-sm normal-case tracking-normal text-haldi" aria-hidden="true">
                  मन
                </span>
                on whatsapp, 24x7
              </p>
              <h2 className="h-display text-4xl !text-ivory md:text-5xl">
                Meet Manu — your <em className="text-sunrise italic">4 a.m. dost</em>
              </h2>
                  ...
                  >
                    ...
                    {i === active && (
                      <motion.span
                        ...
                        style={{
                          background: TAB_HUES[i % TAB_HUES.length],
                          boxShadow: `0 0 12px ${TAB_HUES[i % TAB_HUES.length]}`,
                        }}
                ...
                >
                  {tab.items.map((item) => (
                    <li key={item.title} className="flex gap-5">
                      <span
                        className="mt-3.5 h-[3px] w-12 shrink-0 rounded-full"
                        style={{
                          background: `linear-gradient(90deg, ${TAB_HUES[active % TAB_HUES.length]}, transparent)`,
                        }}
                        aria-hidden="true"
                      />
                      <div>
            ...
            <Reveal from="right" delay={0.15}>
              <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-4 border-t border-white/10 pt-7">
                <a
                  ...
                  className="card-lift inline-flex items-center gap-2.5 rounded-full bg-sunrise bg-[length:200%_100%] bg-left px-6 py-3 text-sm font-semibold text-forest-950 shadow-kesar transition-[background-position] duration-500 ease-silk hover:bg-right"
                >
                ...
                <p className="max-w-xs text-xs leading-relaxed text-sage-light/60">
                  ...
                  <strong className="font-semibold text-haldi">Tele-MANAS, 14416</strong> — free and
                  confidential, 24x7.
                </p>
              </div>
            </Reveal>
